...
:class:`~kaic.matrix.RegionPairsContainer` (see previous section :ref:`edges_interface`),
...
After loading a dataset using :func:`~kaic.tools.load.load`, you can check for
support of the :class:`~kaic.matrix.RegionMatrixContainer` interface with:
...
The current list of Kai-C classes supporting :class:`~kaic.matrix.RegionMatrixContainer` is:
...
:func:`~kaic.matrix.RegionMatrix.matrix` function:
...
Of course, the :func:`~kaic.matrix.RegionMatrixContainer.matrix` function supports matrix subsets:
...
The returned object is of type :class:`~kaic.matrix.RegionMatrix`, which is a subclass
of Numpy's masked :class:`~numpy.ma.array` with added perks for genomic region handling.

A :class:`~kaic.matrix.RegionMatrix` can be used like any other numpy matrix,
...
:class:`~kaic.matrix.RegionMatrix` objects also keep track of the regions corresponding to
...
You can subset a :class:`~kaic.matrix.RegionMatrix` using indexes or region intervals:
...
:func:`~kaic.matrix.RegionMatrixContainer.matrix` supports all arguments also available for
:func:`~kaic.matrix.RegionPairsContainer.edges`, but it is not necessary to use lazy loading.
...
:func:`~kaic.matrix.RegionMatrixContainer.matrix`. Most notably, you can use the
...
:class:`~kaic.matrix.RegionMatrixContainer.expected_values` to calculate the expected
