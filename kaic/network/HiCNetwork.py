...
#             n = self.g.add_vertex()
    ...
    @classmethod
    def liebermannBinnedHiC(cls, fileName, hic, genome=None, resolution=None, fdrIntra=None, fdrInter=None, findInterPeaks=True):
        if type(genome) == str:
...
def liebermannBinnedHiC(cls, hic, genome=None, resolution=None, fdrIntra=None, fdrInter=None, findInterPeaks=True, output=None):
    if type(genome) == str:
        genome = kaic.genome.genomeTools.loadGenomeObject(genome)
...
#             print mergedPeak['list']
    ...
    if output != None:
        with open(output, 'w') as o:
            o.write("obs\tfdr\td\tll\tv\th\td_fdr\tll_fdr\tv_fdr\th_fdr\n")
